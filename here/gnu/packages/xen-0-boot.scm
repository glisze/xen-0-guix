;; xen-0-boot.scm 20191024,gl for an environment to build the xen-tools
;; use:
;;  guix environment --load-path=here xen-0-boot
;;    guix build --load-path=here /xen-0-boot

(define-module (gnu packages xen-0-boot)
...
override CC = " (assoc-ref inputs "cross-gcc") "/bin/i686-linux-gnu-gcc"))
	...
	#;(add-before 'set-environment-up 'our-own-xen-0-config
	  (lambda* (#:key outputs inputs native-inputs #:allow-other-keys)
	    (let ((xen-config (string-append (assoc-ref native-inputs "xen-config") "/xen-0/xen.config")))
	      (if xen-config	     
	  ...
	  (lambda* (#:key outputs #:allow-other-keys)
		   (let* ((out (assoc-ref outputs "out"))
		     ...
		     (invoke "find" dist "-maxdepth" "1"
			     "!" "(" "-name" "gnu" "-or" "-name" "\\.*" "-or" "-name" "install" ")"
			     "-exec" "mv" "-v" "{}" dist-a ";"))))
	(delete 'check)
        (replace 'install
...
;;xen-0-boot ;; guix environment --load-path=here xen-0-boot
