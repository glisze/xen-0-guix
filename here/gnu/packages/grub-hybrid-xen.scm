;;; 20191019 12:50 gl, grub-hybrid-xen.scm --- kernel = ".../pvgrub2.bin"
;;;  this is a development package definition
...
(define-module (gnu packages grub-hybrid-xen)
                     ...
                     (substitute* "util/grub-mkrescue.c"
                       (("\"mcopy\"")
                        (string-append "\"" mtools
                                       "/bin/mcopy\"")))
                     #t))))))))))
(define-public grub-xen
  (package
    (inherit grub)
    (name "grub-xen")
    (synopsis "GRand Unified Boot loader (Xen version)")
    (inputs
     `(("efibootmgr" ,efibootmgr)
       ("mtools" ,mtools)
       ,@(package-inputs grub)))
...
;; minimal changes to grub hybrid
(define-public grub-hybrid-xen
  (package
    (inherit grub-xen)
    (name "grub-hybrid-xen")
    (synopsis "GRand Unified Boot loader (hybrid Xen version)")
    (inputs
     `(("grub" ,grub)
       ("grub-efi" ,grub-efi)
       ,@(package-inputs grub-xen)))
    (arguments
     (substitute-keyword-arguments (package-arguments grub-xen)
       ((#:modules modules `((guix build utils) (guix build gnu-build-system)))
        `((ice-9 ftw) ,@modules))
       ((#:phases phases)
        `(modify-phases ,phases
           (add-after 'install 'install-non-xen
             (lambda* (#:key inputs outputs #:allow-other-keys)
               (let ((grub-input-dir (string-append (assoc-ref inputs "grub")
                                               "/lib/grub"))
		     (grub-efi-input-dir (string-append (assoc-ref inputs "grub-efi")
                                               "/lib/grub"))
                     (output-dir (string-append (assoc-ref outputs "out")
                                                "/lib/grub")))
                 (for-each
                  (lambda (name)
                    (if (not (or (string-prefix? "." name)
                                 (file-exists? (string-append output-dir "/" name))))
                        (symlink (string-append grub-input-dir "/" name)
                                 (string-append output-dir "/" name))))
                  (scandir grub-input-dir))
		 (for-each
                  (lambda (name)
                    (if (not (or (string-prefix? "." name)
                                 (file-exists? (string-append output-dir "/" name))))
                        (symlink (string-append grub-efi-input-dir "/" name)
                                 (string-append output-dir "/" name))))
                  (scandir grub-efi-input-dir))
                 #t)))))))))
...
;; grub-hybrid-xen ;; guix build --file=s/grub-hybrid-xen.scm
